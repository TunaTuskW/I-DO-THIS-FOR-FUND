# 24/7 Macro Analyst Agent Setup (v1.5.0)
...
- **Main Briefing:** `macro briefing (YYYY-MM-DD HH:MM UTC).md`
- **Weekly Synthesis:** `macro weekly synthesis (YYYY-MM-DD UTC).md`
- **Weekly Log Bridge:** `macro_weekly_log.md`
...
6. Append one line to `macro_weekly_log.md` in the following format:
   `[UTC Timestamp] | [Session] | [Sentiment tag] | [Escalation tier] | [Single most important development — one sentence, sourced]`
   Create the file if it does not exist.

7. Update the main title inside the briefing document to reflect the current 
   timestamp. Rename the briefing file to match: 
   `macro briefing (YYYY-MM-DD HH:MM UTC).md`

8. Run the Discord notification script for the newly updated briefing file:
   `python3 push_to_discord.py "macro briefing (YYYY-MM-DD HH:MM UTC).md"`
   
9. Run the Discord notification script for the weekly log file:
   `python3 push_to_discord.py "macro_weekly_log.md"`
```
...
4. Clear macro_weekly_log.md and reset it with a new header line:
   `# Weekly Log — Week of [YYYY-MM-DD]`
   The new week's 4-hour updates will begin populating it from this point.
