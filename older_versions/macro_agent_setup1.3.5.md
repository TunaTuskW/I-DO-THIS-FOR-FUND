# 24/7 Macro Analyst Agent Setup (v1.3.5)
...
Create a file named `macro briefing (YYYY-MM-DD HH:MM UTC).md` with the following initial content:
...
**System Prompt / Instruction for the Background Task (v1.3.5):**
```text
...
0. Find the most recent macro briefing file in the directory (it will be named `macro briefing (YYYY-MM-DD HH:MM UTC).md`). 
   Permanently delete any timestamped sections older than 72 hours from this file.
...
6. Finally, update the main title inside the briefing document to reflect the current timestamp (e.g., `# Macro Briefing (2026-05-22 04:00 UTC)`). Then use your terminal tools to rename the briefing file so that its filename matches the exact current timestamp. For example: `macro briefing (2026-05-22 04:00 UTC).md`.
```
