# 24/7 Macro Analyst Agent Setup (v1.7.2)
...
| File | Discord | Local |
| --- | --- | --- |
| `72 hours roll (timestamp).md` | Pushed (Daily) | Kept |
| `/updates/4 hours update (timestamp).md` | Pushed | Kept |
...
3a. Before writing the update, determine the escalation tier:
   First: read the data_driven_escalation field from market_snapshot.json.
   Use this as the baseline tier. You may escalate further based on
   qualitative developments but may not downgrade below this value.
...
4. Identify which market session this update covers 
   ...
   b. ASSET DASHBOARD — Read market_snapshot.json for all values.
   Use current field for levels, delta_pct for percentage move,
   momentum for trend direction, spread_2s10s for the 2s10s spread.

      CROSS-ASSET CHECK — Read cross_asset_flags from market_snapshot.json.
...
6. Write the same content as a standalone reader file in the /updates/ subfolder:
   `/updates/4 hours update (YYYY-MM-DD HH:MM UTC).md`
...
--- PROCESS ---
1. Find the current rolling document in the directory 
   (named `72 hours roll (YYYY-MM-DD HH:MM UTC).md`).
