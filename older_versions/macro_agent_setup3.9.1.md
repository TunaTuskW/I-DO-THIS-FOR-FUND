# 24/7 Macro Analyst Agent Setup (v3.9.1)
...
3a. Before writing the update, determine the escalation tier.
   ...
   - VIX spikes by >15% in a single session OR crosses above 22
   - DXY daily price change exceeds 0.75% (rapid USD squeeze)
   ...
   - HMM transition_risk > 0.35 alongside MCS < 0
     (early warning: regime transition likely next cycle; not a crisis signal on its own)

   CRITICAL — any one of the following:
   - Equity index move exceeding 2% intraday
   - VIX spikes above 30 (acute market panic)
   - DXY spikes above 104+ with high daily volatility (global liquidity squeeze)
...
3b. Before writing any analysis section, complete all five SAP stages in order.
    ...
    - Direct the agent to evaluate the Structural Ambiguity Index (SAI) and rolling Brier Score calibration.
    - If SAI < 0.30: The HMM and Deep MLP models are in high agreement. The agent must proceed with high conviction.
    - If 0.30 <= SAI < 0.65: The models are in moderate tension. The agent must carefully identify the conflicting asset indicators.
    - If SAI >= 0.65: A Model Divergence Event is active. The agent is strictly commanded to raise an analytical warning flag and explain the exact mechanical cause of the divergence.
    - Brier Score Calibration: If brier_score_calibration > 0.25, the agent must degrade its output confidence level to LOW, reduce risk aggressiveness, and focus heavily on capital preservation.
    ...
    The agent must append a dedicated, concise, non-theatrical section at the end of every update.
    The assumptions must be written in plain, concise, prop-desk style English under the strict 5-part structure:
...
4. Identify which market session this update covers 
   ...
   If spx_weekly_boundaries.swept_pwl_flag or swept_pwh_flag is True, the agent must lead the Risk Flags section with the structural implications of the sweep, incorporating the Regime-Conditioned Success Probability.
...
v3.9.1 Patch Notes (Brutalist UI Cleanup):
Minor aesthetic patch refining the final output format. Simplified headers across the board: [ QUANTITATIVE MATRIX ] -> [ MATRIX ], [ SYSTEM HEALTH ] -> [ STATUS ], [ TACTICAL DIAGNOSTICS ] -> [ DIAGNOSTICS ], and [ CONSENSUS ENGINE ] -> [ SIGNAL ]. Removed the 'Clean Models' internal tracking ratio from the final output. Completely removed the literal string [ RELEVANT HEADLINES ] block to prioritize pure, clinical math logic, although the NewsSignalVector continues to dynamically penalize or reinforce the thresholds in the background.

v3.9.0 Patch Notes (Quantitative News Integration & Consensus Engine):
Elevated news from a decorative text field into a live mathematical vector. Integrated vaderSentiment and feedparser to scrape Yahoo Finance, WSJ, and CNBC. Created a Priority Score sorting algorithm (Authority + Recency Decay + Keyword Relevance). Standardized the 5 core math modules into uniform ModelResult dataclasses. Built the Consensus Engine to strip out noise (Entropy > 1.50) and run a normalized conviction-weighted vote. The NewsSignalVector now dynamically pushes and pulls the validation threshold (from 0.60 down to 0.50 on confirmation, or up to 0.65 on heavy macro uncertainty).

v3.7.0 Patch Notes (The Pipeline Patch):
Fixed silent data drops by expanding yf.download to 30 days to correctly calculate the 20-day rolling institutional Volume Heat. Remapped SPX and VIX hardcoded index tags. Overhauled report generation into a Brutalist Terminal UI layout for cross-platform rendering consistency.

v3.5.0 Patch Notes (Epistemic Sizing & Paradox Fix):
Resolved the 'No Predict' statistical paradox by introducing a dynamically scaling Fractional Kelly Criterion and Shannon Entropy Measurement.
v3.0.0 Patch Notes (Continuous Volume Heat & Robust Diagnostics):
Implemented strict architectural separation between physical volume mechanics and qualitative market extremes. Introduced the continuous Institutional Heat Index (IHI) via Volume Spread Analysis (Z-score * Result Vector) directly into the ML feature vector, eliminating unstable hard thresholds. Introduced 10-day absolute divergence logic for Market Crowdedness and Z-score mapping for Market Temperature. Eliminated redundant Python sentiment proxies, natively delegating sentiment interpretation to the LLM via prompt directives.

v2.9.0 Patch Notes:
Consolidated quantitative release introducing advanced model governance, HMM/MLP cross-divergence tracking via Kullback-Leibler (KL) Divergence, the Structural Ambiguity Index (SAI), self-calibration monitoring via a rolling Brier Score, 1.5 standard deviation dynamic invalidation boundary calculation, and the institutional AI Strategic Assumptions Layer.

v2.8.0 Patch Notes:
Implemented Volatility-Filtered Sweep Ingestion (PWH/PWL), KL Divergence Resolver with Structural Ambiguity Index (SAI), and Dynamic Bayesian Conditioning Engine for regime-filtered historical probability rates. Updated dashboard to display advanced tactical indicators.

v2.7.1 Patch Notes:
Refined build_72h_roll pipeline to separate roll compiling from file deletion, and fully updated workspace directories for path independence.

v2.7.0 Patch Notes:
Consolidated quantitative release merging Data Science Structuring, Silver/USDCAD integration, Gold-to-Silver Ratio, IBIT/ETHA/COIN Spot ETF crypto flow tracking, keyless corporate credit fallbacks, GARCH stability optimizations, TruChain cryptographic signing/verification/hash-chaining, and offline/live Multi-Layer Perceptron (MLP) Deep Classifier training and inference loops.
