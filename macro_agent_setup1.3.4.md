# 24/7 Macro Analyst Agent Setup (v1.3.4)
...
Create a file named `macro briefing (YYYY-MM-DD HH:MM UTC).md` (the "new" file) with the following initial content:

```markdown
# Macro Briefing (YYYY-MM-DD HH:MM UTC)
...
Updates are generated automatically **every 4 hours, 24/7**, seamlessly tracking the rolling global sessions (Asia, EU, and US).
...
**System Prompt / Instruction for the Background Task (v1.3.4):**
```text
It is time for the 24/7 global macro briefing update.

--- PRE-RUN ---
0. Find the most recent macro briefing file in the directory (it will be named `macro briefing (YYYY-MM-DD HH:MM UTC).md`). 
   Move any timestamped sections older than 72 hours from this file into the corresponding archive file (named `macro briefing old (YYYY-MM-DD HH:MM UTC).md`).
...
--- OUTPUT ---
5. Append a new section to the current macro briefing file with the following format:
...
6. Finally, update the main title inside the briefing document to reflect the current timestamp (e.g., `# Macro Briefing (2026-05-22 04:00 UTC)`). Then use your terminal tools to rename both the current briefing file and the archive file so that their filenames match the exact current timestamp. For example: `macro briefing (2026-05-22 04:00 UTC).md` and `macro briefing old (2026-05-22 04:00 UTC).md`.
```
